...
warnings.filterwarnings(
    "ignore", category=DeprecationWarning, message="builtin type .* has no __module__ attribute"
)
...
class ArabicTranslator:
    ...
    def translate_batch(self, texts: list[str]) -> list[tuple[str, str, str]]:
        ...
        Returns:
            list: List of (translated_text, status, error_message) tuples.
        """
        if not texts:
            return []
        ...
        results: list[tuple[str, str, str] | None] = [None] * len(texts)
        ...
        try:
            ...
            for idx, translation in zip(original_indices, decoded_results, strict=True):
